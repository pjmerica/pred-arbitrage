...
- **Fee schedules are in the APIs.**
  - **Kalshi:** `GET /trade-api/v2/series` returns every series (~14k) with `fee_type` and `fee_multiplier`. On 2026-09-24: 14,176 quadratic×1, 159 quadratic_with_maker_fees×1, 18 ×0.5, 14 ×0 (fee-free). Event-level overrides come from `GET /events/fee_changes` (public, one page). On 2026-09-25 it had 255 rows, all scheduled to switch at game start. The scraper applies overrides whose `scheduled_ts` has passed on top of the series fee.
