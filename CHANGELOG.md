...
- *(pending push)* docs: reflect threshold matcher in HANDOFF / AUDIT / NOTES / SCRAPER_NOTES — 7cedd68 added a third matching path but the docs still said "two matching paths". Updated HANDOFF.md (full match_threshold_pairs section: asset list, pattern catalog, PER_CATEGORY_THRESHOLD overrides), AUDIT.md ("Two matching paths" → "Three matching paths", matcher.py file-table row refreshed to ~900 LOC), NOTES_FOR_REVIEWER.md (mental-model + one-hour reading list now mention all three paths), SCRAPER_NOTES.md (pipeline line for matcher.py now lists all three paths). README.md unchanged (only had a generic `python scripts/matcher.py` invocation, no architecture description that needed updating).
